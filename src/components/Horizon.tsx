...
export function Horizon() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const res = await fetch(`https://cors-anywhere.herokuapp.com/https://re.jrc.ec.europa.eu/api/v5_2/printhorizon?lat=${geoLocation[0]}&lon=${geoLocation[1]}&browser=0&outputformat=json&js=1`)
            const data = await res.json()
            setHorizonData(data["outputs"]["horizon_profile"].map((x: any) => {
